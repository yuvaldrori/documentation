...
```yaml
# A standard container, built via a Dockerfile with a custom directory (app)
# as the context for the build. The Dockerfile is searched for at ./app
app:
  build:
    image: codeship/app
    path: app
    dockerfile_path: Dockerfile
# Similar to the above container, but with the configure directive enabled. This
# will run "make compile" in the "app" service but make the result available in
# the deploy container
deploy:
  build:
    image: codeship/app-slim
    dockerfile_path: Dockerfile.slim
    configure:
      service: app
      command: make compile
# The most simple service definition, simply launch a container hosted on the
# Docker Hub (and define a volume for use in other services)
data:
  image: busybox
  volumes:
    - ./tmp/site:/site
