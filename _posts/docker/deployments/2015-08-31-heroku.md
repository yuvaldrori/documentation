...
The container has a `heroku_codeship deploy` command that you need to call with the path to your application, here provided through a host volume in `/deploy`, and your application name. The script will then check that it has access to the application and deploy it.
...
- service: herokudeployment
  command: heroku_codeship deploy /deploy codeship-heroku-deployment
- service: herokudeployment
