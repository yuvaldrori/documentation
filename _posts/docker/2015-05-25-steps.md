...
* A regular expression for more advanced configurations. Please note, that we allow the full range of regular expressions and you need to make sure that the expression only matches those branches you want the step to run on.

    * `tag: (master|staging)` would run a command on any of those branches: `master`, `staging`, `staging-codeship`, `aws-staging`
    * `tag: ^(master|staging)$` would limit the command to only the `master` and `staging` branch.
